...
df_price = pd.DataFrame(dtype='float64')
...
for j in range(0, params['stock_count']):
    ...
    for eps in params['list_epsilon']:
        ...
        negative = 0
        for i in range (0, min_n-1):
            difference = currPrice[i+1]-currPrice[i]
            if (difference>eps):
                positive = positive + 1
    ...
    seriesLabel = pd.Series(index=currPrice.index, name="Label"+str(balEpsilon)+currentFile, dtype='float64')
    for i in range (0, min_n-1):
        difference = currPrice[i+1]-currPrice[i]
    ...
    for i in range(1,params['max_lagging']+1):
        seriesLagged = pd.Series(currPrice.shift(i), index=currPrice.index, name="Lagging "+str(i)+currentFile, dtype='float64')
